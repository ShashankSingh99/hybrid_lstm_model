...
def do_ml(ticker):
   ...
   # clf = neighbors.KNeighborsClassifier()
    ...
    print("Prediction successfull with accuracy %s , Plotting the output " % (confidence))
    return confidence
    
  #  test['predicted_color_as_int'] = clf.predict(X_test)
...
def train(train_x,train_y,test_x,test_y,ticker):
...
def predictFuture(ticker):
    ...
    train(x_train,y_train,x_test,y_test,ticker=ticker)
...
str1=str(input("Enter Company Stock Code"))
do_ml(str1)
